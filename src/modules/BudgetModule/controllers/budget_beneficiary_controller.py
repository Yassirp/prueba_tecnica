...
from modules.BudgetModule.services.budget_beneficiary_services import BudgetBeneficiaryService
...
class BudgetBeneficiaryController:
    ...
    def get_all():
        try:
            ...
            with get_db() as db:
                service = BudgetBeneficiaryService(db)
    ...
    @require_auth
    @require_permission("Acceder", "Presupuesto")
    def get(budget_beneficiary_id: int):
        with get_db() as db:
            try:
                service = BudgetBeneficiaryService(db)
                budget = service.get_by_id(budget_beneficiary_id)
                if not budget:
    ...
    def update(budget_beneficiary_id: int):
        data = request.get_json()
        with get_db() as db:
            try:
                service = BudgetBeneficiaryService(db)
                budget = service.update(budget_beneficiary_id, data)
                if not budget:
                    return jsonify({'error': 'Budget not found'}), 404
                return jsonify(serialize_model(budget))
            except Exception as e:
    ...
    def delete(budget_beneficiary_id: int):
        with get_db() as db:
            try:
                service = BudgetBeneficiaryService(db)
                budget = service.delete(budget_beneficiary_id)
                if not budget:
