...
class BudgetDetailCostController:
    ...
    def get_all():
        try:
            ...
            with get_db() as db:
                service = BudgetDetailCostService(db)
    ...
    @require_auth
    @require_permission("Acceder", "Presupuesto")
    def get(budget_detail_id: int):
        with get_db() as db:
            try:
                service = BudgetDetailCostService(db)
                budget = service.get_by_id(budget_detail_id)
                if not budget:
    ...
    def update(budget_detail_id: int):
        data = request.get_json()
        with get_db() as db:
            try:
                service = BudgetDetailCostService(db)
                budget = service.update(budget_detail_id, data)
                if not budget:
                    return jsonify({'error': 'Budget not found'}), 404
                return jsonify(serialize_model(budget))
            except Exception as e:
    ...
    def delete(budget_detail_id: int):
        with get_db() as db:
            try:
                service = BudgetDetailCostService(db)
                budget = service.delete(budget_detail_id)
                if not budget:
