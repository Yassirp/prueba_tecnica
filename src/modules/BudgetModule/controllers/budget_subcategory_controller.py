...
class BudgetSubcategoryController:
    ...
    @require_auth
    @require_permission("Acceder", "Presupuesto")
    def get(budget_category_id: int):
        with get_db() as db:
            try:
                service = BudgetCategoryService(db)
                budget = service.get_by_id(budget_category_id)
                if not budget:
    ...
    def update(budget_category_id: int):
        data = request.get_json()
        with get_db() as db:
            try:
                service = BudgetCategoryService(db)
                budget = service.update(budget_category_id, data)
                if not budget:
                    return jsonify({'error': 'Budget not found'}), 404
                return jsonify(serialize_model(budget))
            except Exception as e:
    ...
    def delete(budget_category_id: int):
        with get_db() as db:
            try:
                service = BudgetCategoryService(db)
                budget = service.delete(budget_category_id)
                if not budget:
