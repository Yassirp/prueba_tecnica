...
from modules.FlowModule.services.object_state_services import ObjectStateService
...
class ObjectStateController:
    ...
    @staticmethod
    @require_auth
    @require_permission("Acceder", "Estados")
    def get_object_states():
        try:
            limit, offset, order_by = get_pagination_params()
            filter = get_filter_params()
            with get_db() as db:
                service = ObjectStateService(db)
    ...
    @require_auth
    @require_permission("Acceder", "Estados")
    def get_object_state(flow_id: int):
        try:
            with get_db() as db:
                service = ObjectStateService(db)
    ...
    @require_auth
    @staticmethod
    @require_permission("Crear", "Estados")
    ...
    @require_auth
    @staticmethod
    @require_permission("Actualizar", "Estados")
    def update_object_state(flow_id: int):
        data = request.get_json()
        with get_db() as db:
            service = ObjectStateService(db)
            try:
                action = service.update(flow_id, data)
                return jsonify(serialize_model(action)), HTTPStatus.OK
            except ValidationError as e:
                return jsonify({
                    "message": "Error de validación",
                    "errors": e.errors()
    ...
    @require_auth
    @staticmethod
    @require_permission("Eliminar", "Estados")
    def delete_object_state( flow_id: int):
        try:
            with get_db() as db:
                service = ObjectStateService(db)
